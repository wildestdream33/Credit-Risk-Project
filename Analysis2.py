...
# Enhanced helper function to write output with formatting
def write_output(text, file, separator=True):
...
def calculate_statistics(data, label, file):
    ...
    write_output(f"\n### {label} ###", file)
    write_output(f"Total Variance: {variance}", file)
    write_output(f"Skewness:\n{skewness}", file)
    write_output(f"Kurtosis:\n{kurtosis_values}", file)
...
write_output("Loaded dataset preview:\n" + str(df_application.head()), output_file_before_pca)

# Initial shape of the dataset
print("\nInitial Shape of the Dataset:")
print(f"Rows: {df_application.shape[0]}, Columns: {df_application.shape[1]}")
write_output(f"Initial Shape of the Dataset:\nRows: {df_application.shape[0]}, Columns: {df_application.shape[1]}", output_file_before_pca)
...
write_output(f"Shape after dropping high-missing columns:\nRows: {df_application_clean.shape[0]}, Columns: {df_application_clean.shape[1]}", output_file_before_pca)

# Drop remaining rows with missing values
df_application_clean = df_application_clean.dropna()
print("\nShape after dropping missing rows:")
print(f"Rows: {df_application_clean.shape[0]}, Columns: {df_application_clean.shape[1]}")
write_output(f"Shape after dropping missing rows:\nRows: {df_application_clean.shape[0]}, Columns: {df_application_clean.shape[1]}", output_file_before_pca)
...
write_output(f"Shape after removing 'XNA' in CODE_GENDER:\nRows: {df_application_clean2.shape[0]}, Columns: {df_application_clean2.shape[1]}", output_file_before_pca)
...
write_output(f"Shape after one-hot encoding:\nRows: {encoded_application_clean.shape[0]}, Columns: {encoded_application_clean.shape[1]}", output_file_before_pca)

# Standardize the dataset
numeric_columns = df_application_clean2.select_dtypes(include='number').columns.drop(['SK_ID_CURR', 'TARGET'])
...
write_output("Basic Statistics After Standardization:\n" + str(encoded_application_clean_std[numeric_columns].describe()), output_file_before_pca)
...
variance_before_pca, skewness_before_pca, kurtosis_before_pca = calculate_statistics(winsorized_data[numeric_columns], "Before PCA", output_file_before_pca)
...
variance_after_pca, skewness_after_pca, kurtosis_after_pca = calculate_statistics(pd.DataFrame(pca_transformed_optimal), "After PCA", output_file_after_pca)

write_output(f"Number of Components for 95% Variance: {optimal_components}", output_file_after_pca)
write_output(f"Explained Variance by Components:\n{pca_optimal.explained_variance_ratio_}", output_file_after_pca)
write_output(f"Shape after PCA: Rows: {pca_transformed_optimal.shape[0]}, Columns: {pca_transformed_optimal.shape[1]}", output_file_after_pca)

# Print summary
print(f"\nTotal Variance Before PCA: {variance_before_pca}")
print(f"Total Variance After PCA: {variance_after_pca}")
print(f"Shape after PCA: {pca_transformed_optimal.shape}")
